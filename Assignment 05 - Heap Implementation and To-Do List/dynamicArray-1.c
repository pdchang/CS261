/*******************************************************************
** Program Filename: Assignment 5 - dynamicArray.c
** Author: Philip Chang
** Date: March 5, 2017
...
void adjustHeap(DynamicArray* heap, int last, int position, compareFunction compare)
{
	...
	if (right < last) {
		...
		if (compare(dyGet(heap, small), dyGet(heap, position)) < 0) {
			dySwap(heap, position, small);
			adjustHeap(heap, last, small, compare);
		}
	}
	else if (left < last) {
		if (compare(dyGet(heap, left), dyGet(heap, position)) < 0) {
...
void dyHeapRemoveMin(DynamicArray* heap, compareFunction compare)
{
	...
	assert(heap != NULL);
	assert(dySize(heap) > 0);
	int min = dySize(heap) - 1;
	dySwap(heap, 0, min);
	dyRemoveAt(heap, min);
	adjustHeap(heap, min, 0, compare);
